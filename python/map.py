...
class Map:
    ...
    def __generate_height_map(self, min_peaks, max_peaks):
        ...
        index_pairs = [(randint(0, 499), randint(0, 499))
                       for _ in range(randint(min_peaks, max_peaks))]
        ...
        for p in range(len(index_pairs)):
            ...
            while (min_x >= 0 or max_x < 501 or min_y >= 0 or max_y < 501):
                ...
                if p != 0 and (max_x - min_x +
                               1) // 2 > peak_width:  # type: ignore
                    break
